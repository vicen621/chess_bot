use crate::board::{Board, PiecePositions, PieceType, Color, CastlingRights, Square};
...
impl FenParser {
    ...
    pub fn parse(fen: &str) -> Result<Board, FenError> {
        ...
        let pieces_types = FenParser::parse_piece_types(parts[0])?;
        ...
        let halfmove_clock = parts[4].parse::<u32>().map_err(|_| FenError::InvalidFormat)?;
        let fullmove_counter = parts[5].parse::<u32>().map_err(|_| FenError::InvalidFormat)?;

        Ok(Board::new(pieces, pieces_types, turn, castling_rights, en_passant, halfmove_clock, fullmove_counter))
    }
        ...
        for (rank_index, rank) in ranks.iter().enumerate() {
            ...
            for c in rank.chars() {
                if c.is_digit(10) {
                    ...
                    let square = Square::from_algebraic(&format!("{}{}", ('a' as u8 + file_index as u8) as char, (rank_index + 1) as u8))?; // +1 porque las filas empiezan en 1
    ...
    fn parse_piece_types(pieces_fen: &str) -> Result<[PieceType; 64], FenError> {
        let mut pieces_types = [PieceType::Empty; 64];
        let ranks: Vec<&str> = pieces_fen.split('/').collect();
        ...
        for (rank_index, rank) in ranks.iter().enumerate() {
            ...
            for c in rank.chars() {
                if c.is_digit(10) {
                    ...
                    let piece_type = PieceType::from_char(c)?;
                    let square = Square::from_algebraic(&format!("{}{}", ('a' as u8 + file_index as u8) as char, (rank_index + 1) as u8))?; // +1 porque las filas empiezan en 1
                    pieces_types[square.0 as usize] = piece_type;
                    file_index += 1;
                }
            }
...
mod tests {
    ...
    mod tests {
        use super::*;
        use crate::board::{CastlingRights, Color};

        #[test]
        fn test_parse_starting_position() {
            let board = FenParser::parse("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1").unwrap();

            assert_eq!(board.get_pieces().get_white().get_pawns(), 0xFF00);
            assert_eq!(board.get_pieces().get_white().get_knights(), 0x42);
            assert_eq!(board.get_pieces().get_white().get_bishops(), 0x24);
            assert_eq!(board.get_pieces().get_white().get_rooks(), 0x81);
            assert_eq!(board.get_pieces().get_white().get_queens(), 0x8);
            assert_eq!(board.get_pieces().get_white().get_king(), 0x10);

            assert_eq!(board.get_piece_types().get(0).unwrap(), &PieceType::Rook);

            assert_eq!(board.get_turn(), &Color::White);
            assert_eq!(board.get_castling_rights(), &CastlingRights::new(true, true, true, true));
